...
export function parseQuantityFromText(text: string): number | null {
  ...
  const mWord = t.match(/\b(uno|una|un|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\b/);
...
export function detectIntent(text: string): Intent {
  ...
  if (/\b(que\s+tienes|qué\s+tienes|que\s+vendes|catalogo|catálogo|muestrame|muéstrame)\b/.test(t)) {
    return "ask_inventory";
  }
  if (/\b(tienes|hay|manejas|vendes)\b/.test(t)) {
    if (/\b(precio|cuanto\s+cuesta|cuánto\s+cuesta|vale|coste)\b/.test(t)) return "ask_price";
    if (/\b(cuantos|cuantas|cuánto|cuantos\s+te\s+quedan|stock|disponible|disponibles|quedan?)\b/.test(t)) return "ask_stock";
...
export function isStockQuestion(text: string): boolean {
  return /\b(cuantos|cuantas|cuánto|stock|disponible|quedan?)\b/i.test(text);
}
